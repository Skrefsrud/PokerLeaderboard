...
export default async function PlayerStats({ playerId }: { playerId: string }) {
  ...
  const statItems: StatCardItem[] = [
    ...
    {
      label: "Avg/Session (NOK)",
      value: aggregate.avgPerSessionNok.toFixed(0),
    },
    { label: "Win Rate", value: `${aggregate.winRatePct.toFixed(1)}%` },
    { label: "Best Session (NOK)", value: aggregate.bestSessionNok.toFixed(0) },
    {
      label: "Worst Session (NOK)",
      value: aggregate.worstSessionNok.toFixed(0),
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <PerformanceTimelineChart
        data={rolling.map((r) => ({ ...r, date: r.date.toISOString() }))}
      />
      ...
      <BuyInNetScatter
        points={buyInNet.map((p) => ({ ...p, date: p.date.toISOString() }))}
      />
    </div>
  );
}
