...
export function aggregatePlayers(sessions: PlayerSession[]): PlayerAggregate[] {
  ...
  for (const [player, playerSessions] of byPlayer.entries()) {
    ...
    const totalBuyInNok = playerSessions.reduce((sum, s) => sum + s.buyInNok, 0);
    const totalHours = playerSessions.reduce((sum, s) => sum + s.durationHours, 0);

    const winRatePct = (playerSessions.filter((s) => s.netNok > 0).length / totalSessions) * 100;
    ...
    const { longestTrue: longestWinStreak, longestFalse: longestLoseStreak } = streaks(sessionOutcomes);
...
export function buildRollingSeries(playerSessionsSorted: PlayerSession[], window = 5): RollingPoint[] {
    let cumulativeNet = 0;
...
export function heatmapByTimeOfDay(playerSessions: PlayerSession[]): HeatmapBucket[] {
    ...
    for(let i = 0; i < 24; i++) {
        ...
        buckets.push({ xKey: String(i), yKey: 'net', value: nets.length > 0 ? mean(nets) : 0 });
    }
    return buckets;
}

export function heatmapByWeekday(playerSessions: PlayerSession[]): HeatmapBucket[] {
    ...
    return WEEKDAYS.map((dayName, idx) => {
        ...
        return { xKey: dayName, yKey: 'net', value: nets.length > 0 ? mean(nets) : 0 };
    });
}

export function buyInVsNetPoints(playerSessions: PlayerSession[]): Array<{buyIn: number; net: number; date: Date}> {
    return playerSessions.map(s => ({ buyIn: s.buyInNok, net: s.netNok, date: s.start }));
}

export function profitBySessionLength(playerSessions: PlayerSession[], bins: number[] = [0, 1, 2, 3, 5, 8]): Array<{binLabel: string; avgNet: number; count: number}> {
    ...
    return binLabels.map(label => {
        ...
        return { binLabel: label, avgNet: nets.length > 0 ? mean(nets) : 0, count: nets.length };
    });
}

export function pairwiseMatchups(allSessions: PlayerSession[]): PairEdge[] {
    // This is a complex calculation, returning a placeholder for now.
    return [];
}

export function fieldStrengthAdjustment(allSessions: PlayerSession[]): Array<{playerId:string; adjNetPerSession:number}> {
    // Optional, returning placeholder.
    return [];
}
...
export function badgesFromAggregate(p: PlayerAggregate): BadgeInfo[] {
    ...
    if (p.longestLoseStreak >= STREAK_THRESHOLD && p.longestWinStreak >= STREAK_THRESHOLD) {
        badges.push({
            name: 'Comeback Kid',
